...
/***************----- 端口数据缓冲区 -----***************/
unsigned char Rx_Buffer[500];	//端口接收数据缓冲区 
unsigned char Tx_Buffer[500];	//端口发送数据缓冲区 
...
{
	Gateway_IP[0] = 192;        //网关，根据自己的情况而定
	Gateway_IP[1] = 168;
	Gateway_IP[2] = 1;
	Gateway_IP[3] = 1;

	Sub_Mask[0]=255;            //子网掩码，一般都一样，不用动
	Sub_Mask[1]=255;
	Sub_Mask[2]=255;
	Sub_Mask[3]=0;

	Phy_Addr[0]=0x0c;           //MAC地址，无需修改，不用动
	Phy_Addr[1]=0x29;
	Phy_Addr[2]=0xab;
	Phy_Addr[3]=0x7c;
	Phy_Addr[4]=0x00;
	...
	IP_Addr[0]=192;            //开发板IP地址，根据自己的情况而定
	IP_Addr[1]=168;
	IP_Addr[2]=1;
	IP_Addr[3]=89;

	S0_Port[0] = 0x13;        //开发板端口号5000，根据自己的情况而定 
	S0_Port[1] = 0x88;
...
{
	if(S0_State==0)                           //先判断状态变量S0_State=0，表示初始状态
	{
		if(S0_Mode==TCP_SERVER)               //如果选择TCP服务器模式 
		{
			if(Socket_Listen(0)==TRUE)        //服务器设置监听，如果正确返回TRUE
				S0_State=S_INIT;              //标记状态变量S0_State=S_INIT
			else
				S0_State=0;			          //如果没有成功，状态变量S0_State清零
		}
		else if(S0_Mode==TCP_CLIENT)          //如果选择TCP客户端模式 
		{									 
			if(Socket_Connect(0)==TRUE)	      //客户端需要连接远程主机，如果正确返回TRUE
				S0_State=S_INIT;		      //标记状态变量S0_State=S_INIT
			else							  
				S0_State=0;					  //如果没有成功，状态变量S0_State清零
		}
		...
		{
			if(Socket_UDP(0)==TRUE)		      //设置UDP模式，如果正确返回返回TRUE
				S0_State=S_INIT|S_CONN;		  //标记状态变量S0_State=S_INIT|S_CONN   S_INIT：表示初始化成功  S_CONN：表示连接成功，UDP是非连接通信，所以此位无实质作用，同S_INIT保持一致
			else
				S0_State=0;				      //如果没有成功，状态变量S0_State清零
		}
	}
...
void Process_Socket_Data(SOCKET s)
{
	...
	size=Read_SOCK_Data_Buffer(s, Rx_Buffer);   //读取数据，数据量保持到size
  TCP_Cmd_Cjson_Analyze(Rx_Buffer,size);	
	

	memcpy(Tx_Buffer, Rx_Buffer, size);			//把接收的数据，原样的拷贝到Tx_Buffer
	Write_SOCK_Data_Buffer(s, Tx_Buffer, size); //Tx_Buffer数据发送走
